...
class EnrichedWord(BaseModel):
    ...
    collocations: List[TranslatedPhrase] = []
    phrasal_verbs: List[TranslatedPhrase] = []
    idioms: List[TranslatedPhrase] = []
    synonyms: List[str] = []
    ...
    def clean_and_transform_data(cls, data: Any) -> Any:
        if isinstance(data, dict):
            ...
            if isinstance(phonetics_value, dict) and not ('uk' in phonetics_value and isinstance(phonetics_value['uk'], dict)):
